...
{
    public partial class BelowZeroItemSplitSettings : BelowZeroBaseSettings
    {
        ...
        public BelowZeroItemSplitSettings(ItemSplit split)
        {
            InitializeComponent();

            Splits = split.Conditions != null ? split.Conditions.Select(c => c.DeepCopy()).ToList() : new List<BelowZeroSplit>();
            OnlySplitOnce = split.OnlySplitOnce;
            PickUp = split.PickUp;
            Count = split.Count;
            IsCount = split.IsCount;

            ChkSplitOnce.Enabled = !(split.IsSubCondition || BelowZeroSettings.OrderedLiveSplit || BelowZeroSettings.OrderedAutoSplits);
