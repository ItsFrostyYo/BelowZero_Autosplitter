            ...
            foreach (string processName in ProcessNames)
            {
                foreach (var p in Process.GetProcessesByName(processName))
                {
                    try
                    {
                        ...
                        if (!p.MainModule.FileName.EndsWith(processName + ".exe", StringComparison.OrdinalIgnoreCase))
                            continue;
                        ...
                        game = wrapper;
                        logger?.Log($"Process Found. PID: {game.Process.Id}, {(game.Is64Bit ? "64" : "32")}bit, Path: {p.MainModule.FileName}");
                        OnHook?.Invoke();
                        return true;
                    }
                    catch (Exception ex)
